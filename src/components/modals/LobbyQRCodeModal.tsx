  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      ...
      >
        ...
        >
          {t("lobby.joinSession", "Join Challenge Session")}
        </span>
        <h3
        ...
        <p className="text-[12px] text-[#6B7C6B] mb-4">
          {t("lobby.modalScanDesc", "Scan this QR code with your camera or enter room code below")}
        </p>
        ...
        <div className="bg-[#F7FBF5] border border-[#D4ECC5] p-4 rounded-2xl inline-block mb-4 shadow-sm">
          {qrDataUrl ? (
            ...
            <div className="w-52 h-52 flex items-center justify-center text-[#6B7C6B]">
              {t("lobby.generatingQR", "Generating QR...")}
            </div>
          )}
        </div>
        ...
        <div className="bg-[#F0F8EC] border border-[#D4ECC5] py-2 px-4 rounded-xl mb-5 inline-block">
          <span className="text-[12px] text-[#6B7C6B] font-bold mr-2">{t("lobby.roomCodeLabel", "ROOM CODE:")}</span>
          <span
        ...
        <button
          onClick={onClose}
          className="w-full py-3.5 rounded-2xl bg-[#B3D59F] text-[#1A3312] font-extrabold text-[15px] shadow-sm hover:bg-[#9DC885] active:scale-[0.98] transition-all cursor-pointer"
          style={{ fontFamily: "'Lexend', sans-serif" }}
        >
          {t("lobby.closeOpenLobby", "Close & Open Lobby Room")}
        </button>
      </div>
    </div>
  );
